 ...
 * @version v2.0.0
 */
define("fly", ["jquery"], function(require, exports, module){
    ...
    (function(){
        //基类(掺类)
        fly.Class = function(guid) {
            this.guid = guid || fly.guid();
            ...
            this.lib = {};
            /**
             * 方法调用，依次从以下顺序查找 [监听器，当前对象，自定义方法库(this.lib)，公有方法库(fly.lib)，window]
             * @arguments   fun[,...]
            ...
            this.fire = function(fun) {
                var me = this;
                //方法查找范围
                var libs = [me.__listeners, me, me.lib, fly.lib, window];
                if( typeof fun != "undefined") {
                    for(var i = 0, len = libs.length; i < len; i++) {
                        var own = libs[i];
                        var parts = fun.split(".");
                        fly.each(parts, function(name) {
                            own = own[name];
         ...
         */
        fly.each = function(source, fun, thisObj) {
        ...
        /**
         * 类型判断
         * @example  fly.isArray(sth)
         */
        fly.each(["Array", "Boolean", "Date", "Number", "Object", "RegExp", "String", "Window", "HTMLDocument"], function(item, i) {
            fly["is" + item] = function(args) {
        ...
        fly.clone = function() {
            ...
            if(!target || fly.isString(target) || fly.isNumber(target) || fly.isBoolean(target)) {
                return target;
            } else if(fly.isArray(target)) {
                var arr = [];
                for(var i = 0; i < target.length; i++) {
                    arr[i] = deep ? fly.clone(deep, target[i]) : target[i];
                };
                return arr;
            } else if(fly.isObject(target)) {
                var obj = {};
                for(var i in target) {
                    obj[i] = deep ? fly.clone(deep, target[i]) : target[i];
                }
                return obj;
            }
            return {};
        ...
        /**
         * 对象扩充
         * @example fly.extend([true/false,]obj[,obj[...]]);
         */
        fly.extend = function() {
                ...
                for(; i < length; i++) {
                    if(( opts = arguments[i]) != null) {
                        for(name in opts) {
                            ...
                            if(deep && copy && (fly.isArray(copy) || fly.isObject(copy))) {
                                var clone = src && (fly.isArray(src) || fly.isObject(src)) ? src : fly.isArray(copy) ? [] : {};
                                target[name] = fly.extend(deep, clone, copy);
        ...
        fly.extend(fly.Class, {
            ...
            createClass : function(constructor, attr) {
                ...
                var attr = attr || {};
                var opts = fly.extend({}, me._base, attr);

                return fly.createClass(constructor, opts);
            }
        });
    })();
    /**
    ...
    (function(){
         ...
         * 构造Model实体
         * @example
         * 	var Model = fly.Model.createClass(function(){
         *		//constructor
         *		this.name = "";
         *		this.age = "2";
         *	},{
        ...
        fly.extend(fly.Model, {
            _base : {
                ...
                save : function(url, opts) {
                    ...
                    function realFun(opts) {
                        var fun = opts.success;
                        return function(json) {
                            //fly.extend(me.attributes, opts.data);
                            fun.call(this, json);
                        }
                    }
                    opts.success = realFun(opts);
            ...
            extend : function(opts) {
                return new (fly.Model.createClass(function(){},opts))();
            }
        });
    })();
    /**
        ...
        fly.extend(fly.View, {
            _base : {
                ...
                getClass : function(key) {
                    var me = this, className = me.classPrefix || "fly-" + me.uiType.toLowerCase();
                    if(key) {
                        className += "-" + key;
                    }
                    return className;
                ...
                dispose : function() {
                    ...
                    for(var key in me.events){
                        ...
                        fly.die(selector, eventName, me.el);
                    }
                    delete window["$FLYJS$"]._instances[this.guid];
                },
                    ...
                    if(events) {
                        for(var key in events) {
                            ...
                            if(typeof method == "function"){
                                fly.live(selector, eventName, realListener(method), me.el);
                            }else {
            ...
            extend : function(opts) {
                return new (fly.View.createClass(function(){},opts))();
            }
        });
    })();
    /**
    ...
    (function(){
        ...
        fly.extend(fly.Modules, {
            ...
            get : function(name) {
                ...
                fly.Modules.use(arr[1], function(text) {
                    ret = text;
                }, {
                    async : false
                });
                var index = parseInt(arr[3]);
                if( typeof arr[3] != "undefined" && fly.isNumber(index)) {
                    return ret[index];
                }
                return ret;
            },
            ...
            template : function(name, data) {
                return fly.template(fly.Modules.get(name), data);
            }
        });
        fly.addModule = fly.Modules.add;
        fly.useModule = fly.Modules.use;
    ...
    (function() {
        fly.extend(fly.Template, {
            ...
            template : function(str, data) {
                if( typeof fly.Template._tpls[str] != "undefined") {
                    var func = fly.Template._tpls[str];
                    return data ? func(data) : func;
                }
                var tmpl = 'var __p=[],print=function(){__p.push.apply(__p,arguments);};' + 'with(obj||{}){__p.push(\'' + str.replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(fly.Template.interpolate, function(match, code) {
                    var modifier = "", args = []
                    arr = code.split(/[|:]/);
                    code = arr[0];
                    if( typeof arr[1] != "undefined") {
                        if(/^@.+?/.test(arr[1])) {
                            ...
                            modifier = "fly.Template.modifier." + arr[1];
                        }
                ...
                }).replace(fly.Template.evaluate || null, function(match, code) {
                        return "');" + code.replace(/\\'/g, "'").replace(/[\r\n\t]/g, ' ') + "__p.push('";
                    }).replace(/\r/g, '\\r').replace(/\n/g, '\\n').replace(/\t/g, '\\t') + "');}return __p.join('');";
                var func = new Function('obj', tmpl);
                fly.Template._tpls[str] = func;
                return data ? func(data) : func;
            },
            modifier : {
                //编码
        ...
        fly.format = fly.Template.format;
        fly.template = fly.Template.template;
        fly.modifier = fly.Template.modifier;
    })();
    /**
     * 历史管理
     * @author lifayu@baidu.com
    ...
    (function() {
        ...
        fly.extend(fly.History.prototype, {
            ...
            loadUrl : function(fragmentOverride) {
                ...
                fly.each(me.handlers, function(item, i) {
     ...
     * @example
     * var Router = fly.Router.createClass(function(){},{
     *  routes:{
     * 		"!/home/:uid":"home"
     *  },
     *  home:function(uid){
        ...
        fly.extend(fly.Router, {
            _base : {
                ...
                route : function(route, name, callback) {
                    var me = this;
                    fly.history || (fly.history = new fly.History());
                    if(!isRegExp(route)) {
                    ...
                    fly.history.addRouter(route, function(fragment) {
                 ...
                 */
                navigate : function(fragment, triggerRoute) {
                    fly.history.navigate(fragment, triggerRoute);
                },
                //将路由规则转换为正则表达式
            ...
            extend : function(opts) {
                return new (fly.Router.createClass(function(){},opts))();
            }
        });
